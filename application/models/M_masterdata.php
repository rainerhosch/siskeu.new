...
class M_masterdata extends CI_Model
{
    ...
    public function getDataPembayaranChart($where = null, $where_in = null)
    {
        // $this->db->distinct();
        $this->db->select('t.*, td.*');
    ...
    // test function
    public function getDataPembayaranChartV2($where = null)
    {
        // $this->db->distinct();
        $this->db->select('t.*');
        $this->db->from('transaksi t');
    ...
    {
        $this->db->select('id_smt, nm_smt, smt, id_thn_ajaran, a_periode_aktif as status');
        $this->db->from('kalender_akademik');
        if ($filter != null) {
