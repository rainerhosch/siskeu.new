...
class PDF extends FPDF
{
    ...
    function Footer()
    {
        ...
        $this->Cell(70, 5, bilang($_GET['total_bayar']) . 'Rupiah', 1, 0, 'L');
        ...
        $this->Cell(70, 4, '', 0, 0, 'R');
        $this->Cell(30, 4, $_GET['user'], 0, 0, 'C');
        $this->Image('assets/image/ttd/' . $_GET['ttd'], 80, 120, 17);
        $this->Cell(5, 4, '', 0, 0, 'R');
        $this->Cell(30, 4, 'Nama Jelas', 0, 1, 'C');
        $this->SetFont('Arial', 'I', 9);
        $this->Line(7, 140, 140, 138);
        $this->Cell(30, 7, $_GET['tgl_wkt'] . '/trx_ke-' . $_GET['trx_ke'], 0, 0, 'L');
...
$q_reg = mysql_query("SELECT reg_manual,kewajiban_C2,C2, kewajiban_C3, C3, total_sisa_bayar FROM transaksi WHERE id_transaksi='$id_transaksi'");
$data_reg = mysql_fetch_array($q_reg);
...
if (($data_reg['reg_manual'] == 1) or ($data_reg['reg_manual'] == 2 and $data_reg['kewajiban_C2'] == $data_reg['C2'])) {
    ...
    $pdf->RotatedText(173, 117, ': ' . $tampil['Nama'] . ' / ' . $tampil['id_transaksi'], 90);

    $pdf->RotatedText(178, 140, 'NIM/Jurusan', 90);
    $pdf->RotatedText(178, 117, ': ' . $tampil['nim'] . ' / ' . $_GET['jur'], 90);
    ...
    $pdf->RotatedImage('assets/image/ttd/' . $_GET['ttd'], 193, 45, 15, 9, 90);
...
//$pdf->Cell(70,4,'Telp : (0264) 214952 | Fax (0264) 822 5153',0,1,'L');
//$pdf->Line(128,11,128,26);
$pdf->SetFont('Arial', 'BU', 11);
...
$pdf->Cell(30, 4, ': ' . $tampil['id_transaksi'], 0, 0, 'L');
$pdf->Cell(15, 4, 'Nama', 0, 0, 'L');
$pdf->Cell(70, 4, ': ' . substr($tampil['Nama'], 0, 25), 0, 1, 'L');
$pdf->Cell(25, 4, 'NIM', 0, 0, 'L');
$pdf->Cell(30, 4, ': ' . $tampil['nim'], 0, 0, 'L');
...
if ($tampil['kewajiban_TG'] != 0) {
    //$pdf->Cell(66,5,'Tunggakan',1,0,'L');
    $pdf->Cell(66, 5, 'Sisa Pembayaran Semester', 1, 0, 'L');
    $pdf->Cell(24, 5, number_format($tampil['kewajiban_TG'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['TG'], 0, '', '.'), 1, 0, 'R');

    $pdf->Cell(20, 5, number_format($tampil['sisa_TG'], 0, '', '.'), 1, 1, 'R');
}
//UANG BANGUNAN
if ($tampil['kewajiban_UB'] != 0) {
    $pdf->Cell(66, 5, 'Uang Bangunan', 1, 0, 'L');
    $pdf->Cell(24, 5, number_format($tampil['kewajiban_UB'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['UB'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['sisa_UB'], 0, '', '.'), 1, 1, 'R');
}
//KEMAHASISWAAN
if ($tampil['kewajiban_kmhs'] != 0) {
    $pdf->Cell(66, 5, 'Kemahasiswaan', 1, 0, 'L');
    $pdf->Cell(24, 5, number_format($tampil['kewajiban_kmhs'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['kmhs'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['sisa_kmhs'], 0, '', '.'), 1, 1, 'R');
}
//CICILAN SEMESTER

if ($tampil['kewajiban_CS'] != 0) {
    //Jika Bayaran dilunasi
    /*
		if($tampil['lunasi_CS']!=0){
			$jml_byr_cs = $tampil['lunasi_CS'];
			$pdf->Cell(90,4,'Cicilan Semester : (DILUNASI)',1,0,'L');
			$pdf->Cell(40,4,number_format($tampil['kewajiban_CS'],0,'','.'),1,0,'R');
			$pdf->Cell(30,4,number_format($jml_byr_cs,0,'','.'),1,0,'R');
			$pdf->Cell(30,4,number_format($tampil['sisa_CS'],0,'','.'),1,1,'R');
		}
		*/
    //end Jika Bayaran dilunasi
    //Jika Bayaran dicicil
    //else{
    $akademikAktif_tampil = substr($tampil['idtahun'], 0, -1);
    $akademikAktif_tampilNext = $akademikAktif_tampil + 1;
    $akademikAktif_cek = substr($tampil['idtahun'], 4);
    if ($akademikAktif_cek == 1) {
        $smt = "Ganjil";
    } else {
        $smt = "Genap";
    }
    $t_akad = $akademikAktif_tampil . "/$akademikAktif_tampilNext " . $smt;
    $jml_byr_cs = $tampil['C1'] + $tampil['C2'] + $tampil['C3'];
    $pdf->Cell(66, 5, 'Cicilan Smstr : (' . $t_akad . ')', 1, 0, 'L');
    $pdf->Cell(24, 5, number_format($tampil['kewajiban_CS'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($jml_byr_cs, 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['sisa_CS'], 0, '', '.'), 1, 1, 'R');
    $pdf->SetFont('Arial', 'I', 10);
    if ($tampil['C1'] != 0) {
        $pdf->Cell(66, 5, '#Cicilan Ke-1 @Bayar Rp. ' . number_format($tampil['C1'], 0, '', '.'), 1, 0, 'L');
        $pdf->Cell(24, 5, '', 1, 0, 'R');
        $pdf->Cell(20, 5, '', 1, 0, 'R');
        $pdf->Cell(20, 5, '', 1, 1, 'R');
    }
    if ($tampil['C2'] != 0) {
        $pdf->Cell(66, 5, '#Cicilan Ke-2 @Bayar Rp. ' . number_format($tampil['C2'], 0, '', '.'), 1, 0, 'L');
        $pdf->Cell(24, 5, '', 1, 0, 'R');
        $pdf->Cell(20, 5, '', 1, 0, 'R');
        $pdf->Cell(20, 5, '', 1, 1, 'R');
    }
    if ($tampil['C3'] != 0) {
        $pdf->Cell(66, 5, '#Cicilan Ke-3 @Bayar Rp. ' . number_format($tampil['C3'], 0, '', '.'), 1, 0, 'L');
        $pdf->Cell(24, 5, '', 1, 0, 'R');
        $pdf->Cell(20, 5, '', 1, 0, 'R');
        $pdf->Cell(20, 5, '', 1, 1, 'R');
    }
...
$pdf->SetFont('Arial', '', 11);
//SEMINAR KP
if ($tampil['kewajiban_EK'] != 0) {
    $pdf->Cell(66, 5, 'Seminar KP', 1, 0, 'L');
    $pdf->Cell(24, 5, number_format($tampil['kewajiban_EK'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['EK'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['sisa_EK'], 0, '', '.'), 1, 1, 'R');
}
//JUDUL PROPOSAL TA
if ($tampil['kewajiban_PT'] != 0) {
    $pdf->Cell(66, 5, 'Judul TA', 1, 0, 'L');
    $pdf->Cell(24, 5, number_format($tampil['kewajiban_PT'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['PT'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['sisa_PT'], 0, '', '.'), 1, 1, 'R');
}
//SEMINAR TA
if ($tampil['kewajiban_ET'] != 0) {
    $pdf->Cell(66, 5, 'Seminar TA', 1, 0, 'L');
    $pdf->Cell(24, 5, number_format($tampil['kewajiban_ET'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['ET'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['sisa_ET'], 0, '', '.'), 1, 1, 'R');
}
//SIDANG TA
if ($tampil['kewajiban_IT'] != 0) {
    $pdf->Cell(66, 5, 'Sidang TA', 1, 0, 'L');
    $pdf->Cell(24, 5, number_format($tampil['kewajiban_IT'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['IT'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['sisa_IT'], 0, '', '.'), 1, 1, 'R');
}
//BIAYA LAINNYA
if ($tampil['kewajiban_BL'] != 0) {
    $pdf->Cell(66, 5, 'Biaya Lainnya (' . $tampil['keterangan_bl'] . ')', 1, 0, 'L');
    $pdf->Cell(24, 5, number_format($tampil['kewajiban_BL'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['BL'], 0, '', '.'), 1, 0, 'R');
    $pdf->Cell(20, 5, number_format($tampil['sisa_BL'], 0, '', '.'), 1, 1, 'R');
}
//Total
$pdf->SetFont('Arial', 'B', 11);
$pdf->Cell(66, 5, 'Total', 1, 0, 'R');
$pdf->Cell(24, 5, number_format($tampil['total_kewajiban'], 0, '', '.'), 1, 0, 'R');
$pdf->Cell(20, 5, number_format($tampil['total_bayar'], 0, '', '.'), 1, 0, 'R');
$pdf->Cell(20, 5, number_format($tampil['total_sisa_bayar'], 0, '', '.'), 1, 1, 'R');
